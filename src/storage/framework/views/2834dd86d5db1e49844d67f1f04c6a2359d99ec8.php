    ...
    headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
		...
		success: function (res) {
			if(res.status == 'exceed'){
				swal("Something Happened To Stock", "<?php echo app('translator')->getFromJson('website.Ops! Product is available in stock But Not Active For Sale. Please contact to the admin'); ?>", "error");
			}
			else{
				jQuery('.head-cart-content').html(res);
				jQuery(parent).removeClass('cart');
		...
		jQuery(function ($) {
			...
			jQuery.ajax({
				...
				success: function (res) {
					...
					showData.push("<option value='-1'><?php echo app('translator')->getFromJson('website.Other'); ?></option>");
...
<?php /**PATH /var/www/html/farm2homewebnew/farm2homewebnew/src/resources/views/web/common/scripts.blade.php ENDPATH**/ ?>