...
class Customer extends Model
{
    ...
    public static function processlogin($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            $customerInfo = array("phone" => $email, "password" => $password, 'role_id' => 2);
    ...
    public static function processregistration($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            $existUser = DB::table('users')->where(function($query) use ($email, $customers_telephone){
                $query->orWhere('email', $email)
                    ->orWhere('phone', $customers_telephone);
            })->get();
    ...
    public static function processforgotpassword($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            $existUser = DB::table('users')->where('phone', $email)->get();
            ...
            if (count($existUser) > 0) {
                ...
                $responseData = array('success' => '1', 'data' => $postData, 'message' => "Your password has been successfully change.");
                // $responseData = array('success' => '1', 'data' => $postData, 'message' => "Your password has been sent to your email address.");
            } else {
                $responseData = array('success' => '0', 'data' => $postData, 'message' => "Phone address doesn't exist!");
            }
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
    ...
    public static function facebookregistration($request)
    {
        ...
        if ($authenticate) {
            ...
            if (count($existUser) > 0) {
                ...
                if(!empty($email)){
                    $emailUsers = DB::table('users')->where('email', '=', $email)->get();
                    if (count($emailUsers) > 0){
                        DB::table('customers')->insertGetId([
                            'fb_id' => $fb_id,
                            'user_id' => $emailUsers[0]->id,
                        ]);
                    }else{
