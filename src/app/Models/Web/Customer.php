  ...
  public function signupProcess($request){
      ...
      $data = array(
        ...
        'updated_at' => $date,
                ...
                $user_email = DB::table('users')->whereNotNull('email')->select('email')->where('role_id', 2)->where('email', $email)->get();
                if(count($user_email)>0){
                  ...
                  if(DB::table('users')->insert([
                    ...
                    if(
                      ...
                      auth()->guard('customer')->attempt(['phone' => $request->phone, 'password' => $request->password])){
                      $res['auth'] = "true";
                      $customer = auth()->guard('customer')->user();
                      //set session
                      session(['customers_id' => $customer->id]);
