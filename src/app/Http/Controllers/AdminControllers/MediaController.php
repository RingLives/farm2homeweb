...
class MediaController extends Controller
{
    ...
    public function add()
    {
        // print str_replace("src", "", base_path());die();
        $Images = new Images();
    ...
    public function storeThumbnial($Path,$filename,$directory,$input)
    {
      ...
      $time = time();
        ...
        $destinationPath = str_replace("src", "", base_path())."images/media/".$directory."/";
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
        ...
        $Path = $destinationPath .'thumbnail'.$time.$filename;
        $destinationFile = $Path;
        $mainImg = "images/media/".$directory."/thumbnail".$time.$filename;
        $size = getimagesize($destinationFile);
        list($width, $height, $type, $attr) = $size;
        $Images = new Images();
        $storethumbnail = $Images->thumbnailrecord($input,$mainImg,$width,$height);
...
//        $height = $Medium_values[0]->value;
        ...
        $Path = $destinationPath .'medium'.$time.$filename;

        $destinationFile = $Path;
        $size = getimagesize($destinationFile);
        list($width, $height, $type, $attr) = $size;

        $mainImg = "images/media/".$directory."/medium".$time.$filename;
        $storeMediumImage = $Images->Mediumrecord($input,$mainImg,$width,$height);
...
//        $upload_success = $thumbnailImage->save($directory, $filename, 'public');
        ...
        $Path = $destinationPath.'large'.$time.$filename;
        $destinationFile = $Path;
        $size = getimagesize($destinationFile);
        list($width, $height, $type, $attr) = $size;

        $mainImg = "images/media/".$directory."/large".$time.$filename;
        $storeLargeImage = $Images->Largerecord($input,$mainImg,$width,$height);
