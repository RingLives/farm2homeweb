   ...
   session(['guest_checkout' => 1]);
	...
	public function checkout(Request $request){
		...
		if(count($result['cart'])==0){
			...
			if(isset($result['address'])){
				foreach ($result['address'] as $address_data) {
					if($address_data->default_address == 1)
					{
						...
						$address->delivery_phone=auth()->guard('customer')->user()->phone;
					}
				}
			}
			...
			// print_r("<pre>");
			// print_r(session()->get('shipping_address'));die();
...
// 			}
	...
	public function checkout_shipping_address(Request $request){
		...
		if(isset($request->shiping_address_check) && $request->shiping_address_check == "other"){
			$this->shipping->addMyAddress($request);
		}
        
        // print_r(count(Arr::wrap($request->shipping_address_list)));die();
        if(isset($request->shipping_address_list) && count(Arr::wrap($request->shipping_address_list)) == 1){
...
// 		die("shohid");
	...
	public function shipping_methods(){
		...
		if(!empty(session('shipping_address'))){		
			$countries_id = session('shipping_address')->countries_id ?? 12;
