	...
	public function signupProcess(Request $request){
		...
		$validator = Validator::make(
			array(
				...
				'phone' => str_replace(['88', '+88'], '', $request->get('phone')),
				'password' => $request->password,
				...
				'phone' 	=> 'required|regex:/(01)[0-9]{9}/|phone_number|min:11|max:11|unique:users',
				'password'  => 'required',
				're_password' => 'required|same:password',
			)
		);
		if($validator->fails()){
			return redirect('login')->withErrors($validator->messages())->withInput($request->all());
		}else{
		...
		$res = $this->customer->signupProcess($request);
			...
			if($res['email'] == "true"){
				...
				return redirect('/login')->withInput($request->input())->with('error', "Phone number already exist");
				...
				if( $res['insert'] == "true"){
					if($res['auth'] == "true"){
						...
						{
							return redirect()->action('VerifiedController@sendCode', [
								'verify' => auth()->guard(User::CUSTOMER_GUARD)->user()->phone ?? '',
							]);
						}else{
							$result = $res['result'];
							return redirect()->intended('/')->with('result', $result);
